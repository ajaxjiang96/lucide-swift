...
/// Italic Square icon
internal enum LucideLabIcon_Lab_ItalicSquare {
    
/// Italic Square icon path (combined)
static let combinedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 16.0, y: 7.0))
path.addLine(to: CGPoint(x: 10.0, y: 7.0))
path.move(to: CGPoint(x: 13.5, y: 7.0))
...
/// Italic Square icon open subpaths
...
/// Italic Square icon closed subpaths
