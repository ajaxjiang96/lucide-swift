//
//  Icon_HardHat.swift
//  LucideSwift
//
//  Auto-generated — DO NOT EDIT
//  Icon: hard-hat
//

import SwiftUI

/// Hard Hat icon
internal enum LucideIcon_HardHat {
    
/// Hard Hat icon path (combined)
static let combinedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 10.0, y: 10.0))
path.addLine(to: CGPoint(x: 10.0, y: 5.0))
path.addCurve(to: CGPoint(x: 11.0, y: 4.0), control1: CGPoint(x: 10.0, y: 4.447715250169207), control2: CGPoint(x: 10.447715250169207, y: 4.0))
path.addLine(to: CGPoint(x: 13.0, y: 4.0))
path.addCurve(to: CGPoint(x: 14.0, y: 5.0), control1: CGPoint(x: 13.552284749830793, y: 4.0), control2: CGPoint(x: 14.0, y: 4.447715250169207))
path.addLine(to: CGPoint(x: 14.0, y: 10.0))
path.move(to: CGPoint(x: 14.0, y: 6.0))
path.addCurve(to: CGPoint(x: 20.0, y: 12.0), control1: CGPoint(x: 17.31370849898476, y: 6.0), control2: CGPoint(x: 20.0, y: 8.686291501015239))
path.addLine(to: CGPoint(x: 20.0, y: 15.0))
path.move(to: CGPoint(x: 4.0, y: 15.0))
path.addLine(to: CGPoint(x: 4.0, y: 12.0))
path.addCurve(to: CGPoint(x: 9.999999999999998, y: 6.0), control1: CGPoint(x: 4.0, y: 8.68629150101524), control2: CGPoint(x: 6.686291501015238, y: 6.000000000000001))
path.move(to: CGPoint(x: 3.0, y: 15.0))
path.addLine(to: CGPoint(x: 21.0, y: 15.0))
path.addCurve(to: CGPoint(x: 22.0, y: 16.0), control1: CGPoint(x: 21.552284749830793, y: 15.0), control2: CGPoint(x: 22.0, y: 15.447715250169207))
path.addLine(to: CGPoint(x: 22.0, y: 18.0))
path.addCurve(to: CGPoint(x: 21.0, y: 19.0), control1: CGPoint(x: 22.0, y: 18.552284749830793), control2: CGPoint(x: 21.552284749830793, y: 19.0))
path.addLine(to: CGPoint(x: 3.0, y: 19.0))
path.addCurve(to: CGPoint(x: 2.0, y: 18.0), control1: CGPoint(x: 2.4477152501692068, y: 19.0), control2: CGPoint(x: 2.0, y: 18.552284749830793))
path.addLine(to: CGPoint(x: 2.0, y: 16.0))
path.addCurve(to: CGPoint(x: 3.0, y: 15.0), control1: CGPoint(x: 2.0, y: 15.447715250169207), control2: CGPoint(x: 2.4477152501692063, y: 15.0))
path.closeSubpath()
    return path
}()
/// Hard Hat icon open subpaths
static let openPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 10.0, y: 10.0))
path.addLine(to: CGPoint(x: 10.0, y: 5.0))
path.addCurve(to: CGPoint(x: 11.0, y: 4.0), control1: CGPoint(x: 10.0, y: 4.447715250169207), control2: CGPoint(x: 10.447715250169207, y: 4.0))
path.addLine(to: CGPoint(x: 13.0, y: 4.0))
path.addCurve(to: CGPoint(x: 14.0, y: 5.0), control1: CGPoint(x: 13.552284749830793, y: 4.0), control2: CGPoint(x: 14.0, y: 4.447715250169207))
path.addLine(to: CGPoint(x: 14.0, y: 10.0))
path.move(to: CGPoint(x: 14.0, y: 6.0))
path.addCurve(to: CGPoint(x: 20.0, y: 12.0), control1: CGPoint(x: 17.31370849898476, y: 6.0), control2: CGPoint(x: 20.0, y: 8.686291501015239))
path.addLine(to: CGPoint(x: 20.0, y: 15.0))
path.move(to: CGPoint(x: 4.0, y: 15.0))
path.addLine(to: CGPoint(x: 4.0, y: 12.0))
path.addCurve(to: CGPoint(x: 9.999999999999998, y: 6.0), control1: CGPoint(x: 4.0, y: 8.68629150101524), control2: CGPoint(x: 6.686291501015238, y: 6.000000000000001))
    return path
}()
/// Hard Hat icon closed subpaths
static let closedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 3.0, y: 15.0))
path.addLine(to: CGPoint(x: 21.0, y: 15.0))
path.addCurve(to: CGPoint(x: 22.0, y: 16.0), control1: CGPoint(x: 21.552284749830793, y: 15.0), control2: CGPoint(x: 22.0, y: 15.447715250169207))
path.addLine(to: CGPoint(x: 22.0, y: 18.0))
path.addCurve(to: CGPoint(x: 21.0, y: 19.0), control1: CGPoint(x: 22.0, y: 18.552284749830793), control2: CGPoint(x: 21.552284749830793, y: 19.0))
path.addLine(to: CGPoint(x: 3.0, y: 19.0))
path.addCurve(to: CGPoint(x: 2.0, y: 18.0), control1: CGPoint(x: 2.4477152501692068, y: 19.0), control2: CGPoint(x: 2.0, y: 18.552284749830793))
path.addLine(to: CGPoint(x: 2.0, y: 16.0))
path.addCurve(to: CGPoint(x: 3.0, y: 15.0), control1: CGPoint(x: 2.0, y: 15.447715250169207), control2: CGPoint(x: 2.4477152501692063, y: 15.0))
path.closeSubpath()
    return path
}()

}
