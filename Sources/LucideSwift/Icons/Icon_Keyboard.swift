...
/// Keyboard icon
internal enum LucideIcon_Keyboard {
    
/// Keyboard icon path (combined)
static let combinedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 10.0, y: 8.0))
path.move(to: CGPoint(x: 10.01, y: 8.0))
path.addLine(to: CGPoint(x: 10.01, y: 8.0))
