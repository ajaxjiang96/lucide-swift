...
/// Folder X icon
internal enum LucideIcon_FolderX {
    
/// Folder X icon path (combined)
...
path.closeSubpath()
path.move(to: CGPoint(x: 9.5, y: 10.5))
path.addLine(to: CGPoint(x: 14.5, y: 15.5))
path.move(to: CGPoint(x: 14.5, y: 10.5))
path.addLine(to: CGPoint(x: 9.5, y: 15.5))
    return path
}()
/// Folder X icon open subpaths
