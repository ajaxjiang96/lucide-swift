//
//  Icon_Laugh.swift
//  LucideSwift
//
//  Auto-generated — DO NOT EDIT
//  Icon: laugh
//

import SwiftUI

/// Laugh icon
internal enum LucideIcon_Laugh {
    
/// Laugh icon path (combined)
static let combinedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 18.0, y: 13.0))
path.addCurve(to: CGPoint(x: 12.000000000000002, y: 18.0), control1: CGPoint(x: 17.506994079908257, y: 15.91841995585412), control2: CGPoint(x: 14.959479712659942, y: 18.041348595227714))
path.addCurve(to: CGPoint(x: 5.999999999999998, y: 13.0), control1: CGPoint(x: 9.04052028734006, y: 18.041348595227717), control2: CGPoint(x: 6.493005920091742, y: 15.91841995585412))
path.addLine(to: CGPoint(x: 18.0, y: 13.0))
path.closeSubpath()
path.move(to: CGPoint(x: 2.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 22.0), control1: CGPoint(x: 2.0000000000000018, y: 17.522847498307936), control2: CGPoint(x: 6.477152501692067, y: 22.0))
path.addCurve(to: CGPoint(x: 22.0, y: 12.0), control1: CGPoint(x: 17.522847498307936, y: 22.0), control2: CGPoint(x: 22.0, y: 17.522847498307932))
path.addCurve(to: CGPoint(x: 12.0, y: 2.0), control1: CGPoint(x: 22.0, y: 6.477152501692066), control2: CGPoint(x: 17.522847498307936, y: 2.0))
path.addCurve(to: CGPoint(x: 2.0, y: 11.999999999999998), control1: CGPoint(x: 6.477152501692067, y: 2.0), control2: CGPoint(x: 2.0000000000000018, y: 6.477152501692064))
path.move(to: CGPoint(x: 9.0, y: 9.0))
path.addEllipse(in: CGRect(x: 8.66, y: 8.65, width: 0.7, height: 0.7))
path.move(to: CGPoint(x: 15.0, y: 9.0))
path.addEllipse(in: CGRect(x: 14.66, y: 8.65, width: 0.7, height: 0.7))
    return path
}()
/// Laugh icon open subpaths
static let openPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 9.0, y: 9.0))
path.addEllipse(in: CGRect(x: 8.66, y: 8.65, width: 0.7, height: 0.7))
path.move(to: CGPoint(x: 15.0, y: 9.0))
path.addEllipse(in: CGRect(x: 14.66, y: 8.65, width: 0.7, height: 0.7))
    return path
}()
/// Laugh icon closed subpaths
static let closedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 18.0, y: 13.0))
path.addCurve(to: CGPoint(x: 12.000000000000002, y: 18.0), control1: CGPoint(x: 17.506994079908257, y: 15.91841995585412), control2: CGPoint(x: 14.959479712659942, y: 18.041348595227714))
path.addCurve(to: CGPoint(x: 5.999999999999998, y: 13.0), control1: CGPoint(x: 9.04052028734006, y: 18.041348595227717), control2: CGPoint(x: 6.493005920091742, y: 15.91841995585412))
path.addLine(to: CGPoint(x: 18.0, y: 13.0))
path.closeSubpath()
path.move(to: CGPoint(x: 2.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 22.0), control1: CGPoint(x: 2.0000000000000018, y: 17.522847498307936), control2: CGPoint(x: 6.477152501692067, y: 22.0))
path.addCurve(to: CGPoint(x: 22.0, y: 12.0), control1: CGPoint(x: 17.522847498307936, y: 22.0), control2: CGPoint(x: 22.0, y: 17.522847498307932))
path.addCurve(to: CGPoint(x: 12.0, y: 2.0), control1: CGPoint(x: 22.0, y: 6.477152501692066), control2: CGPoint(x: 17.522847498307936, y: 2.0))
path.addCurve(to: CGPoint(x: 2.0, y: 11.999999999999998), control1: CGPoint(x: 6.477152501692067, y: 2.0), control2: CGPoint(x: 2.0000000000000018, y: 6.477152501692064))
    return path
}()

}
