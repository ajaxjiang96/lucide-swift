//
//  Icon_RefreshCcwDot.swift
//  LucideSwift
//
//  Auto-generated — DO NOT EDIT
//  Icon: refresh-ccw-dot
//

import SwiftUI

/// Refresh Ccw Dot icon
internal enum LucideIcon_RefreshCcwDot {
    
/// Refresh Ccw Dot icon path (combined)
static let combinedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 21.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 3.0), control1: CGPoint(x: 21.0, y: 7.029437251522859), control2: CGPoint(x: 16.970562748477143, y: 3.0))
path.addCurve(to: CGPoint(x: 5.259999999999999, y: 5.739999999999998), control1: CGPoint(x: 9.483951926346212, y: 3.009465100560824), control2: CGPoint(x: 7.068969047726007, y: 3.991223718872302))
path.addLine(to: CGPoint(x: 3.0, y: 8.0))
path.move(to: CGPoint(x: 3.0, y: 3.0))
path.addLine(to: CGPoint(x: 3.0, y: 8.0))
path.addLine(to: CGPoint(x: 8.0, y: 8.0))
path.move(to: CGPoint(x: 3.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 21.0), control1: CGPoint(x: 3.0000000000000018, y: 16.970562748477143), control2: CGPoint(x: 7.02943725152286, y: 21.0))
path.addCurve(to: CGPoint(x: 18.740000000000002, y: 18.259999999999998), control1: CGPoint(x: 14.516048073653788, y: 20.990534899439176), control2: CGPoint(x: 16.931030952273993, y: 20.008776281127695))
path.addLine(to: CGPoint(x: 21.0, y: 16.0))
path.move(to: CGPoint(x: 16.0, y: 16.0))
path.addLine(to: CGPoint(x: 21.0, y: 16.0))
path.addLine(to: CGPoint(x: 21.0, y: 21.0))
path.move(to: CGPoint(x: 11.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 13.0), control1: CGPoint(x: 11.0, y: 12.552284749830793), control2: CGPoint(x: 11.447715250169207, y: 13.0))
path.addCurve(to: CGPoint(x: 13.0, y: 12.0), control1: CGPoint(x: 12.552284749830793, y: 13.0), control2: CGPoint(x: 13.0, y: 12.552284749830793))
path.addCurve(to: CGPoint(x: 12.0, y: 11.0), control1: CGPoint(x: 13.0, y: 11.447715250169207), control2: CGPoint(x: 12.552284749830793, y: 11.0))
path.addCurve(to: CGPoint(x: 11.0, y: 12.0), control1: CGPoint(x: 11.447715250169207, y: 11.0), control2: CGPoint(x: 11.0, y: 11.447715250169207))
    return path
}()
/// Refresh Ccw Dot icon open subpaths
static let openPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 21.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 3.0), control1: CGPoint(x: 21.0, y: 7.029437251522859), control2: CGPoint(x: 16.970562748477143, y: 3.0))
path.addCurve(to: CGPoint(x: 5.259999999999999, y: 5.739999999999998), control1: CGPoint(x: 9.483951926346212, y: 3.009465100560824), control2: CGPoint(x: 7.068969047726007, y: 3.991223718872302))
path.addLine(to: CGPoint(x: 3.0, y: 8.0))
path.move(to: CGPoint(x: 3.0, y: 3.0))
path.addLine(to: CGPoint(x: 3.0, y: 8.0))
path.addLine(to: CGPoint(x: 8.0, y: 8.0))
path.move(to: CGPoint(x: 3.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 21.0), control1: CGPoint(x: 3.0000000000000018, y: 16.970562748477143), control2: CGPoint(x: 7.02943725152286, y: 21.0))
path.addCurve(to: CGPoint(x: 18.740000000000002, y: 18.259999999999998), control1: CGPoint(x: 14.516048073653788, y: 20.990534899439176), control2: CGPoint(x: 16.931030952273993, y: 20.008776281127695))
path.addLine(to: CGPoint(x: 21.0, y: 16.0))
path.move(to: CGPoint(x: 16.0, y: 16.0))
path.addLine(to: CGPoint(x: 21.0, y: 16.0))
path.addLine(to: CGPoint(x: 21.0, y: 21.0))
    return path
}()
/// Refresh Ccw Dot icon closed subpaths
static let closedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 11.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 13.0), control1: CGPoint(x: 11.0, y: 12.552284749830793), control2: CGPoint(x: 11.447715250169207, y: 13.0))
path.addCurve(to: CGPoint(x: 13.0, y: 12.0), control1: CGPoint(x: 12.552284749830793, y: 13.0), control2: CGPoint(x: 13.0, y: 12.552284749830793))
path.addCurve(to: CGPoint(x: 12.0, y: 11.0), control1: CGPoint(x: 13.0, y: 11.447715250169207), control2: CGPoint(x: 12.552284749830793, y: 11.0))
path.addCurve(to: CGPoint(x: 11.0, y: 12.0), control1: CGPoint(x: 11.447715250169207, y: 11.0), control2: CGPoint(x: 11.0, y: 11.447715250169207))
    return path
}()

}
