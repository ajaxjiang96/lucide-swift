//
//  Icon_RobotVacuum.swift
//  LucideSwift
//
//  Auto-generated — DO NOT EDIT
//  Icon: robot-vacuum
//

import SwiftUI

/// Robot Vacuum icon
internal enum LucideIcon_RobotVacuum {
    
/// Robot Vacuum icon path (combined)
static let combinedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 11.0, y: 17.0))
path.addLine(to: CGPoint(x: 13.0, y: 17.0))
path.move(to: CGPoint(x: 12.0, y: 12.0))
path.addLine(to: CGPoint(x: 12.01, y: 12.0))
path.move(to: CGPoint(x: 17.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 7.0), control1: CGPoint(x: 17.0, y: 9.238576250846034), control2: CGPoint(x: 14.761423749153968, y: 7.0))
path.addCurve(to: CGPoint(x: 7.0, y: 12.0), control1: CGPoint(x: 9.238576250846034, y: 7.0), control2: CGPoint(x: 7.000000000000001, y: 9.238576250846032))
path.move(to: CGPoint(x: 19.0, y: 2.0))
path.addLine(to: CGPoint(x: 19.0, y: 4.8))
path.move(to: CGPoint(x: 2.0, y: 5.0))
path.addLine(to: CGPoint(x: 4.8, y: 5.0))
path.move(to: CGPoint(x: 22.0, y: 5.0))
path.addLine(to: CGPoint(x: 19.2, y: 5.0))
path.move(to: CGPoint(x: 5.0, y: 2.0))
path.addLine(to: CGPoint(x: 5.0, y: 4.8))
path.move(to: CGPoint(x: 2.0, y: 12.0))
path.addCurve(to: CGPoint(x: 11.999999999999998, y: 22.0), control1: CGPoint(x: 2.0, y: 17.522847498307932), control2: CGPoint(x: 6.477152501692063, y: 22.0))
path.addCurve(to: CGPoint(x: 22.0, y: 12.000000000000002), control1: CGPoint(x: 17.522847498307932, y: 22.0), control2: CGPoint(x: 22.0, y: 17.522847498307936))
path.addCurve(to: CGPoint(x: 12.0, y: 2.0), control1: CGPoint(x: 22.0, y: 6.477152501692066), control2: CGPoint(x: 17.522847498307936, y: 2.0))
path.addCurve(to: CGPoint(x: 2.0, y: 11.999999999999998), control1: CGPoint(x: 6.477152501692067, y: 2.0), control2: CGPoint(x: 2.0000000000000018, y: 6.477152501692064))
    return path
}()
/// Robot Vacuum icon open subpaths
static let openPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 11.0, y: 17.0))
path.addLine(to: CGPoint(x: 13.0, y: 17.0))
path.move(to: CGPoint(x: 12.0, y: 12.0))
path.addLine(to: CGPoint(x: 12.01, y: 12.0))
path.move(to: CGPoint(x: 17.0, y: 12.0))
path.addCurve(to: CGPoint(x: 12.0, y: 7.0), control1: CGPoint(x: 17.0, y: 9.238576250846034), control2: CGPoint(x: 14.761423749153968, y: 7.0))
path.addCurve(to: CGPoint(x: 7.0, y: 12.0), control1: CGPoint(x: 9.238576250846034, y: 7.0), control2: CGPoint(x: 7.000000000000001, y: 9.238576250846032))
path.move(to: CGPoint(x: 19.0, y: 2.0))
path.addLine(to: CGPoint(x: 19.0, y: 4.8))
path.move(to: CGPoint(x: 2.0, y: 5.0))
path.addLine(to: CGPoint(x: 4.8, y: 5.0))
path.move(to: CGPoint(x: 22.0, y: 5.0))
path.addLine(to: CGPoint(x: 19.2, y: 5.0))
path.move(to: CGPoint(x: 5.0, y: 2.0))
path.addLine(to: CGPoint(x: 5.0, y: 4.8))
    return path
}()
/// Robot Vacuum icon closed subpaths
static let closedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 2.0, y: 12.0))
path.addCurve(to: CGPoint(x: 11.999999999999998, y: 22.0), control1: CGPoint(x: 2.0, y: 17.522847498307932), control2: CGPoint(x: 6.477152501692063, y: 22.0))
path.addCurve(to: CGPoint(x: 22.0, y: 12.000000000000002), control1: CGPoint(x: 17.522847498307932, y: 22.0), control2: CGPoint(x: 22.0, y: 17.522847498307936))
path.addCurve(to: CGPoint(x: 12.0, y: 2.0), control1: CGPoint(x: 22.0, y: 6.477152501692066), control2: CGPoint(x: 17.522847498307936, y: 2.0))
path.addCurve(to: CGPoint(x: 2.0, y: 11.999999999999998), control1: CGPoint(x: 6.477152501692067, y: 2.0), control2: CGPoint(x: 2.0000000000000018, y: 6.477152501692064))
    return path
}()

}
