//
//  Icon_Table2.swift
//  LucideSwift
//
//  Auto-generated — DO NOT EDIT
//  Icon: table-2
//

import SwiftUI

/// Table 2 icon
internal enum LucideIcon_Table2 {
    
/// Table 2 icon path (combined)
static let combinedPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 9.0, y: 3.0))
path.addLine(to: CGPoint(x: 5.0, y: 3.0))
path.addCurve(to: CGPoint(x: 3.0, y: 5.0), control1: CGPoint(x: 3.8954305003384135, y: 3.0), control2: CGPoint(x: 3.0, y: 3.8954305003384126))
path.addLine(to: CGPoint(x: 3.0, y: 9.0))
path.move(to: CGPoint(x: 15.0, y: -3.0))
path.addLine(to: CGPoint(x: 25.0, y: -3.0))
path.addCurve(to: CGPoint(x: 27.0, y: -1.0), control1: CGPoint(x: 26.104569499661586, y: -3.0), control2: CGPoint(x: 27.0, y: -2.104569499661587))
path.addLine(to: CGPoint(x: 27.0, y: 3.0))
path.move(to: CGPoint(x: 9.0, y: 3.0))
path.addLine(to: CGPoint(x: 9.0, y: 21.0))
path.move(to: CGPoint(x: 9.0, y: 3.0))
path.addLine(to: CGPoint(x: 19.0, y: 3.0))
path.addCurve(to: CGPoint(x: 21.0, y: 1.0), control1: CGPoint(x: 20.104569499661586, y: 3.0), control2: CGPoint(x: 21.0, y: 2.104569499661587))
path.addLine(to: CGPoint(x: 21.0, y: 9.0))
path.move(to: CGPoint(x: 9.0, y: 21.0))
path.addLine(to: CGPoint(x: 5.0, y: 21.0))
path.addCurve(to: CGPoint(x: 3.0, y: 19.0), control1: CGPoint(x: 3.8954305003384135, y: 21.0), control2: CGPoint(x: 3.0, y: 20.104569499661586))
path.addLine(to: CGPoint(x: 3.0, y: 9.0))
path.move(to: CGPoint(x: 9.0, y: 21.0))
path.addLine(to: CGPoint(x: 27.0, y: 21.0))
    return path
}()
/// Table 2 icon open subpaths
static let openPath: Path = {
    var path = Path()
    path.move(to: CGPoint(x: 9.0, y: 3.0))
path.addLine(to: CGPoint(x: 5.0, y: 3.0))
path.addCurve(to: CGPoint(x: 3.0, y: 5.0), control1: CGPoint(x: 3.8954305003384135, y: 3.0), control2: CGPoint(x: 3.0, y: 3.8954305003384126))
path.addLine(to: CGPoint(x: 3.0, y: 9.0))
path.move(to: CGPoint(x: 15.0, y: -3.0))
path.addLine(to: CGPoint(x: 25.0, y: -3.0))
path.addCurve(to: CGPoint(x: 27.0, y: -1.0), control1: CGPoint(x: 26.104569499661586, y: -3.0), control2: CGPoint(x: 27.0, y: -2.104569499661587))
path.addLine(to: CGPoint(x: 27.0, y: 3.0))
path.move(to: CGPoint(x: 9.0, y: 3.0))
path.addLine(to: CGPoint(x: 9.0, y: 21.0))
path.move(to: CGPoint(x: 9.0, y: 3.0))
path.addLine(to: CGPoint(x: 19.0, y: 3.0))
path.addCurve(to: CGPoint(x: 21.0, y: 1.0), control1: CGPoint(x: 20.104569499661586, y: 3.0), control2: CGPoint(x: 21.0, y: 2.104569499661587))
path.addLine(to: CGPoint(x: 21.0, y: 9.0))
path.move(to: CGPoint(x: 9.0, y: 21.0))
path.addLine(to: CGPoint(x: 5.0, y: 21.0))
path.addCurve(to: CGPoint(x: 3.0, y: 19.0), control1: CGPoint(x: 3.8954305003384135, y: 21.0), control2: CGPoint(x: 3.0, y: 20.104569499661586))
path.addLine(to: CGPoint(x: 3.0, y: 9.0))
path.move(to: CGPoint(x: 9.0, y: 21.0))
path.addLine(to: CGPoint(x: 27.0, y: 21.0))
    return path
}()
/// Table 2 icon closed subpaths
static let closedPath: Path = {
    var path = Path()
            return path
}()

}
